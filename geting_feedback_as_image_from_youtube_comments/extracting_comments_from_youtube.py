...
import making_ImageFeedback as mif
...
if __name__ == "__main__":
        ...
        with open("texts\{}.txt".format(file_name), "w", encoding='utf-8') as file:
                
                for count, comment in enumerate(get_comment(url)):
                    com_info = dict(comment)
                    file.write(str(com_info['comment'].replace(',', ' ').replace('\n', ''))+"\n")
        print()
        print("done..")

        
        processed_texts = tp.processedText(file_name)
        mif.wordcloud_Func(processed_texts, file_name)
        mif.showFeedback(file_name)
